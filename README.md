...
This project uses a Vue frontend and a FastAPI backend to upload audio files or MP4 videos, send audio to the OpenAI transcription API, and return a downloadable transcript with speaker-assigned turns such as `Speaker 1` and `Speaker 2`.
...
1. Drop an audio file or MP4 video onto the frontend.
2. The frontend posts the file to `POST /api/transcribe`.
3. If the upload is an MP4 video, the backend extracts the first audio track into an audio-only M4A file.
4. The backend sends audio to OpenAI using the configured API key.
5. If the source audio is longer than the diarization model limit, the backend splits the original recording into overlapping chunks and transcribes them in sequence.
6. If an individual chunk is still larger than OpenAI's 25 MB upload limit, that chunk is compressed before upload.
7. OpenAI returns diarized transcript segments.
8. The backend uses the overlap window to stitch chunk-local speakers across boundaries, offsets timestamps back onto the original timeline, and normalizes the speaker labels to `Speaker 1`, `Speaker 2`, and so on.
9. The UI renders speaker-separated turns and exposes the formatted transcript as a `.txt` download.
